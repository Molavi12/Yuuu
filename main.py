from telethon import TelegramClient
from telethon.tl.functions.account import UpdateProfileRequest
import asyncio
from datetime import datetime, timedelta
import os
import base64
import aiohttp
import random

# اطلاعات API شما
api_id = 26600960
api_hash = '73746434553a3b392291b51a49cd41fc'

# کد base64 فایل session که در GitHub آپلود کردید
SESSION_BASE64 = """
"""

# لیست احادیث (می‌توانید از GitHub دریافت کنید یا همین لیست را استفاده کنید)
HADITHS = [
    "النَّظَافَةُ مِنَ الإِيمَانِ - پاکیزگی بخشی از ایمان است",
    "اطْلُبُوا الْعِلْمَ مِنَ الْمَهْدِ إِلَى اللَّحْدِ - از گهواره تا گور دانش بجویید",
    "الْكَلِمَةُ الطَّيِّبَةُ صَدَقَةٌ - سخن نیک صدقه است",
    "تَبَسُّمُكَ فِي وَجْهِ أَخِيكَ صَدَقَةٌ - لبخندت به روی برادرت صدقه است",
    "الْحِكْمَةُ ضَالَّةُ الْمُؤْمِنِ - حکمت گمشده مؤمن است",
    "خَیْرُ النَّاسِ أَنْفَعُهُمْ لِلنَّاسِ - بهترین مردم کسی است که برای مردم سودمندتر باشد",
    "الصَّبْرُ مِفْتَاحُ الْفَرَجِ - شادی کلید گشایش است",
    "الْعِلْمُ نُورٌ - دانش نور است",
    "الْإِيمَانُ مَعْرِفَةٌ بِالْقَلْبِ - ایمان شناخت با دل است",
    "رِضَا اللَّهِ فِي رِضَا الْوَالِدَيْنِ - خشنودی خدا در خشنودی پدر و مادر است"
]

# لینک GitHub برای دریافت احادیث (اختیاری)
HADITHS_GITHUB_URL = "https://github.com/Molavi12/K/blob/main/hadiths.txt"

async def fetch_hadiths_from_github():
    """دریافت احادیث از ریپازیتوری GitHub"""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(HADITHS_GITHUB_URL) as response:
                if response.status == 200:
                    text = await response.text()
                    hadiths = [line.strip() for line in text.split('\n') if line.strip()]
                    return hadiths
                else:
                    print(f"❌ Error fetching hadiths: {response.status}")
                    return HADITHS
    except Exception as e:
        print(f"❌ Error fetching hadiths from GitHub: {e}")
        return HADITHS

def calculate_birthday_countdown():
    # تاریخ تولد: دوم بهمن (22 ژانویه در تقویم میلادی)
    birth_day = 22
    birth_month = 1  # ژانویه
    
    now = datetime.now()
    current_year = now.year
    
    # تاریخ تولد امسال
    birthday_this_year = datetime(current_year, birth_month, birth_day)
    
    # اگر تاریخ تولد امسال گذشته، برای سال بعد حساب کن
    if now > birthday_this_year:
        birthday_this_year = datetime(current_year + 1, birth_month, birth_day)
    
    # محاسبه تفاوت زمان
    time_left = birthday_this_year - now
    
    # تبدیل به روز، ساعت و دقیقه
    days = time_left.days
    hours = time_left.seconds // 3600
    minutes = (time_left.seconds % 3600) // 60
    total_minutes = (days * 24 * 60) + (hours * 60) + minutes
    
    return days, hours, minutes, total_minutes

async def update_time():
    try:
        print("🟢 Starting Telegram Live Clock with Birthday Countdown and Hadiths...")
        
        # دریافت احادیث از GitHub یا استفاده از لیست پیش‌فرض
        hadiths = await fetch_hadiths_from_github()
        print(f"📖 Loaded {len(hadiths)} hadiths")
        
        # ایجاد فایل session از base64 اگر وجود ندارد
        if not os.path.exists('session_name.session'):
            print("📦 Creating session file from base64...")
            
            if SESSION_BASE64.strip() == "PASTE_THE_BASE64_CODE_HERE" or not SESSION_BASE64.strip():
                print("❌ Please replace SESSION_BASE64 with your actual base64 code")
                return
            
            try:
                # حذف spaces و newlines از کد base64
                clean_base64 = SESSION_BASE64.strip().replace('\n', '').replace(' ', '')
                
                # decode base64 و ایجاد فایل session
                session_data = base64.b64decode(clean_base64)
                
                with open('session_name.session', 'wb') as f:
                    f.write(session_data)
                
                file_size = os.path.getsize('session_name.session')
                print(f"✅ Session file created! Size: {file_size} bytes")
                
            except Exception as decode_error:
                print(f"❌ Error decoding base64: {decode_error}")
                print("📋 Please check if the base64 code is complete and correct")
                return
        
        # بررسی وجود فایل session
        if os.path.exists('session_name.session'):
            file_size = os.path.getsize('session_name.session')
            print(f"📁 Using session file, Size: {file_size} bytes")
        else:
            print("❌ Session file not found after creation attempt")
            return
        
        async with TelegramClient('session_name', api_id, api_hash) as client:
            print("✅ Connected to Telegram successfully!")
            
            stickers = ["🏓🥇", "🏓🥈", "🏓🥉"]
            sticker_index = 0
            hadith_index = 0
            
            while True:
                try:
                    # محاسبه زمان باقی‌مانده تا تولد
                    days, hours, minutes, total_minutes = calculate_birthday_countdown()
                    
                    # زمان ایران
                    utc_time = datetime.utcnow()
                    iran_time = utc_time + timedelta(hours=3, minutes=30)
                    current_time = iran_time.strftime('%H:%M')
                    current_sticker = stickers[sticker_index]
                    
                    # انتخاب حدیث
                    current_hadith = hadiths[hadith_index]
                    
                    # ایجاد متن نمایشی با شمارش معکوس و حدیث
                    if days > 30:
                        display_name = f"{current_time} 🎂{days}d {current_sticker}"
                    elif days > 0:
                        display_name = f"{current_time} 🎂{days}d{hours}h {current_sticker}"
                    elif hours > 0:
                        display_name = f"{current_time} 🎂{hours}h{minutes}m {current_sticker}"
                    else:
                        display_name = f"{current_time} 🎂{minutes}m {current_sticker}"
                    
                    print(f"🔄 Updating to: {display_name}")
                    print(f"📜 Hadith: {current_hadith}")
                    
                    # به روزرسانی نام پروفایل و بیو (حدیث)
                    await client(UpdateProfileRequest(
                        first_name=display_name,
                        last_name="",
                        about=current_hadith
                    ))
                    
                    print(f'✅ Updated: {display_name}')
                    print(f'📅 Time until birthday: {days} days, {hours} hours, {minutes} minutes')
                    print(f'📖 Current hadith: {current_hadith}')
                    
                    # تغییر استیکر و حدیث
                    sticker_index = (sticker_index + 1) % len(stickers)
                    hadith_index = (hadith_index + 1) % len(hadiths)
                    
                    await asyncio.sleep(60)
                    
                except Exception as e:
                    print(f'❌ Error: {e}')
                    await asyncio.sleep(60)
                    
    except Exception as e:
        print(f'🚨 Critical Error: {e}')

if __name__ == "__main__":
    asyncio.run(update_time())
